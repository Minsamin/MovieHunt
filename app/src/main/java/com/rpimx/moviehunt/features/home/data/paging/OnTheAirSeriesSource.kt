...
class OnTheAirSeriesSource(private val api: Api) : PagingSource<Int, TvSeries>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TvSeries> {
        return try {
            ...
            LoadResult.Page(
                data = onAirSeries.results, prevKey = if (nextPage == 1) null else nextPage - 1, nextKey = if (onAirSeries.results.isEmpty()) null else onAirSeries.page + 1
            )
