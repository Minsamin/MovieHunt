...
class PopularSeriesSourceTest {

    @MockK
    private lateinit var api: Api

    private lateinit var source: PopularSeriesSource

    @Before
    fun setup() {
        MockKAnnotations.init(this)
        source = PopularSeriesSource(api)
    }
    ...
    fun `test load returns correct LoadResult`() = runTest {
        ...
        coEvery { api.getPopularTvSeries(1) } returns mockResponse
    ...
    fun `test load handles API exceptions`() = runTest {
        // Given
        coEvery { api.getPopularTvSeries(1) } throws IOException("Network error")
