...
class TopRatedSeriesSourceTest {

    @MockK
    private lateinit var api: Api

    private lateinit var source: TopRatedSeriesSource

    @Before
    fun setup() {
        MockKAnnotations.init(this)
        source = TopRatedSeriesSource(api)
    }
    ...
    fun `test load returns correct LoadResult`() = runTest {
        ...
        coEvery { api.getTopRatedTvSeries(1) } returns mockResponse
    ...
    fun `test load handles API exceptions`() = runTest {
        // Given
        coEvery { api.getTopRatedTvSeries(1) } throws IOException("Network error")
