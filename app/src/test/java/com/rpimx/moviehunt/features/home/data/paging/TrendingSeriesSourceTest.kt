...
class TrendingSeriesSourceTest {

    @MockK
    private lateinit var api: Api

    private lateinit var source: TrendingSeriesSource

    @Before
    fun setup() {
        MockKAnnotations.init(this)
        source = TrendingSeriesSource(api)
    }
    ...
    fun `test load returns correct LoadResult`() = runTest {
        ...
        coEvery { api.getTrendingTvSeries(1) } returns mockResponse
    ...
    fun `test load handles API exceptions`() = runTest {
        // Given
        coEvery { api.getTrendingTvSeries(1) } throws IOException("Network error")
