...
class AiringTodayTvSeriesSourceTest {

    @MockK
    private lateinit var api: Api

    private lateinit var source: AiringTodayTvSeriesSource

    @Before
    fun setup() {
        MockKAnnotations.init(this)
        source = AiringTodayTvSeriesSource(api)
    }
    ...
    fun `test load returns correct LoadResult`() = runTest {
        ...
        coEvery { api.getAiringTodayTvSeries(1) } returns mockResponse
    ...
    fun `test load handles API exceptions`() = runTest {
        // Given
        coEvery { api.getAiringTodayTvSeries(1) } throws IOException("Network error")
