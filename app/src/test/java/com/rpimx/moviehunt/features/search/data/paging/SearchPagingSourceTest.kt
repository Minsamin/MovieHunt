...
class SearchPagingSourceTest {
    ...
    private lateinit var source: SearchPagingSource

    @Before
    fun setup() {
        MockKAnnotations.init(this)
        source = SearchPagingSource(api, "")
    }

    @Test
    fun `test load returns correct LoadResult`() = runTest {
        // Given
        val mockResponse = SearchResponse(searches = getData(), page = 1, totalResults = 10, totalPages = 5)
        coEvery { api.search(1, "") } returns mockResponse
    ...
    fun `test load handles API exceptions`() = runTest {
        // Given
        coEvery { api.search(1, "") } throws IOException("Network error")
