...
class TopRatedMoviesSourceTest {
    ...
    private lateinit var source: TopRatedMoviesSource

    @Before
    fun setup() {
        MockKAnnotations.init(this)
        source = TopRatedMoviesSource(api)
    }
    ...
    fun `test load returns correct LoadResult`() = runTest {
        ...
        coEvery { api.getTopRatedMovies(1) } returns mockResponse
    ...
    fun `test load handles API exceptions`() = runTest {
        // Given
        coEvery { api.getTopRatedMovies(1) } throws IOException("Network error")
