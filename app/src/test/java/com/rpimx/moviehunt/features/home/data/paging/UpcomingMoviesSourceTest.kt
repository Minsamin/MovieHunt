...
class UpcomingMoviesSourceTest {
    ...
    private lateinit var source: UpcomingMoviesSource

    @Before
    fun setup() {
        MockKAnnotations.init(this)
        source = UpcomingMoviesSource(api)
    }
    ...
    fun `test load returns correct LoadResult`() = runTest {
        ...
        coEvery { api.getUpcomingMovies(1) } returns mockResponse
    ...
    fun `test load handles API exceptions`() = runTest {
        // Given
        val upcomingMoviesSource = UpcomingMoviesSource(api)
        coEvery { api.getUpcomingMovies(1) } throws IOException("Network error")

        // When
        val result = upcomingMoviesSource.load(PagingSource.LoadParams.Refresh(key = null, loadSize = 20, placeholdersEnabled = false))
