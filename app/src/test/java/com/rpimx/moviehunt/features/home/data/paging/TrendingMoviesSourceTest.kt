...
class TrendingMoviesSourceTest {

    @MockK
    private lateinit var api: Api

    private lateinit var source: TrendingMoviesSource

    @Before
    fun setup() {
        MockKAnnotations.init(this)
        source = TrendingMoviesSource(api)
    }
    ...
    fun `test load returns correct LoadResult`() = runTest {
        ...
        coEvery { api.getTrendingTodayMovies(1) } returns mockResponse
    ...
    fun `test load handles API exceptions`() = runTest {
        // Given
        coEvery { api.getTrendingTodayMovies(1) } throws IOException("Network error")
